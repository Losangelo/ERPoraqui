import { PrismaClient, PerfilUsuario, TipoPessoa, OrigemMercadoria, RegimeTributario, SituacaoPedido, SituacaoOrcamento, SituacaoConta, SituacaoBoleto, FormaPagamento, TipoFluxoCaixa } from '@prisma/client';
...
async function main() {
  ...
  const usersToCreate = [
    { email: 'gerente@erporaqui.com.br', nome: 'Gerente', perfil: 'GERENTE', senha: 'gerente123' },
    { email: 'vendedor@erporaqui.com.br', nome: 'Vendedor', perfil: 'USUARIO', senha: 'venda123' },
    { email: 'financeiro@erporaqui.com.br', nome: 'Financeiro', perfil: 'USUARIO', senha: 'financeiro123' },
  ];
    ...
    if (!existingUser) {
      ...
      await prisma.usuario.create({
        data: {
          ...
          nome: userData.nome,
          senha: hashedPassword,
          perfil: userData.perfil as PerfilUsuario,
          ativo: true
        }
      });
      console.log('✅ Usuario created:', userData.email);
    } else {
      console.log('✅ Usuario already exists:', userData.email);
    }
  }

  // Create Unidades de Medida (only if not exist)
  ...
  // Create Categorias (only if not exist)
  ...
  // Create Clientes (only if not exist)
  ...
  if (existingClientes === 0) {
    await Promise.all([
      prisma.cliente.create({
        data: {
          empresaId: empresa.id, nome: 'João Silva Santos', tipoPessoa: TipoPessoa.FISICA, documento: '123.456.789-00',
      ...
      prisma.cliente.create({
        data: {
          empresaId: empresa.id, nome: 'Maria Oliveira Souza', tipoPessoa: TipoPessoa.FISICA, documento: '987.654.321-00',
      ...
      prisma.cliente.create({
        data: {
          empresaId: empresa.id, nome: 'Empresa ABC Ltda', tipoPessoa: TipoPessoa.JURIDICA, documento: '12.345.678/0001-00',
  ...
  // Create Fornecedores (only if not exist)
  ...
  if (existingFornecedores === 0) {
    await Promise.all([
      prisma.fornecedor.create({
        data: {
          empresaId: empresa.id, nome: 'Fornecedor Tech Ltda', tipoPessoa: TipoPessoa.JURIDICA, documento: '11.111.111/0001-11',
      ...
      prisma.fornecedor.create({
        data: {
          empresaId: empresa.id, nome: 'Distribuidora Nacional SA', tipoPessoa: TipoPessoa.JURIDICA, documento: '22.222.222/0001-22',
  ...
  // Create Vendedores (only if not exist)
  const existingVendedores = await prisma.vendedor.count({ where: { empresaId: empresa.id } });

  if (existingVendedores === 0) {
    await Promise.all([
      prisma.vendedor.create({ data: { empresaId: empresa.id, nome: 'Carlos Alberto Souza', tipoPessoa: TipoPessoa.FISICA, documento: '111.222.333-44', telefone: '(11) 97777-1111', telefoneCelular: '(11) 97777-1111', email: 'carlos.alberto@erporaqui.com.br', comissao: 5.0, ativo: true } }),
      prisma.vendedor.create({ data: { empresaId: empresa.id, nome: 'Ana Paula Rodrigues', tipoPessoa: TipoPessoa.FISICA, documento: '222.333.444-55', telefone: '(11) 97777-2222', telefoneCelular: '(11) 97777-2222', email: 'ana.paula@erporaqui.com.br', comissao: 4.5, ativo: true } }),
    ]);
  ...
  // Create Produtos (only if not exist)
  const existingProdutos = await prisma.produto.count({ where: { empresaId: empresa.id } });

  if (existingProdutos === 0) {
    await Promise.all([
      prisma.produto.create({ data: { empresaId: empresa.id, codigoInterno: 'ELE-001', codigoBarras: '7891234560012', gtin: '7891234560012', nome: 'Notebook Dell Inspiron 15', descricao: 'Notebook Dell Inspiron 15 3000', descricaoDetalhada: 'Intel Core i5, 8GB RAM, 256GB SSD, Tela 15.6"', categoriaId: catInformatica.id, unidadeMedidaId: un.id, precoVenda: 3500.00, precoCusto: 2800.00, precoMinimo: 3200.00, quantidadeEstoque: 15, estoqueMinimo: 5, estoqueMaximo: 50, ncm: '8471.30.90', cest: '01.069.00', origenMercadoria: OrigemMercadoria.NACIONAL, ativo: true } }),
      prisma.produto.create({ data: { empresaId: empresa.id, codigoInterno: 'ELE-002', codigoBarras: '7891234560029', gtin: '7891234560029', nome: 'Mouse Wireless Microsoft', descricao: 'Mouse sem fio Microsoft Sculpt', categoriaId: catInformatica.id, unidadeMedidaId: un.id, precoVenda: 150.00, precoCusto: 80.00, precoMinimo: 120.00, quantidadeEstoque: 50, estoqueMinimo: 10, estoqueMaximo: 100, ncm: '8471.60.70', cest: '01.069.00', origenMercadoria: OrigemMercadoria.NACIONAL, ativo: true } }),
      prisma.produto.create({ data: { empresaId: empresa.id, codigoInterno: 'ELE-003', codigoBarras: '7891234560036', gtin: '7891234560036', nome: 'Teclado Mecânico RGB', descricao: 'Teclado mecânico com iluminação RGB', categoriaId: catInformatica.id, unidadeMedidaId: un.id, precoVenda: 450.00, precoCusto: 280.00, precoMinimo: 380.00, quantidadeEstoque: 25, estoqueMinimo: 5, estoqueMaximo: 50, ncm: '8471.60.60', cest: '01.069.00', origenMercadoria: OrigemMercadoria.ESTRANGEIRA_IMPORTACAO_DIRETA, ativo: true } }),
      prisma.produto.create({ data: { empresaId: empresa.id, codigoInterno: 'ACC-001', codigoBarras: '7891234560043', gtin: '7891234560043', nome: 'Fone de Ouvido Bluetooth', descricao: 'Fone de ouvido sem fio Bluetooth 5.0', categoriaId: catAcessorios.id, unidadeMedidaId: un.id, precoVenda: 250.00, precoCusto: 120.00, precoMinimo: 200.00, quantidadeEstoque: 40, estoqueMinimo: 10, estoqueMaximo: 80, ncm: '8518.30.00', cest: '01.058.00', origenMercadoria: OrigemMercadoria.NACIONAL, ativo: true } }),
      prisma.produto.create({ data: { empresaId: empresa.id, codigoInterno: 'ACC-002', codigoBarras: '7891234560050', gtin: '7891234560050', nome: 'Carregador Universal', descricao: 'Carregador universal 65W USB-C', categoriaId: catAcessorios.id, unidadeMedidaId: un.id, precoVenda: 180.00, precoCusto: 90.00, precoMinimo: 140.00, quantidadeEstoque: 60, estoqueMinimo: 15, estoqueMaximo: 100, ncm: '8504.40.10', cest: '01.075.00', origenMercadoria: OrigemMercadoria.ESTRANGEIRA_NACIONALIZADA, ativo: true } }),
      prisma.produto.create({ data: { empresaId: empresa.id, codigoInterno: 'MOV-001', codigoBarras: '7891234560074', gtin: '7891234560074', nome: 'Cadeira Gamer Pro', descricao: 'Cadeira gamer reclinável com apoio lumbar', categoriaId: catMoveis.id, unidadeMedidaId: un.id, precoVenda: 1200.00, precoCusto: 750.00, precoMinimo: 1000.00, quantidadeEstoque: 10, estoqueMinimo: 3, estoqueMaximo: 20, ncm: '9401.61.00', cest: '01.004.00', origenMercadoria: OrigemMercadoria.NACIONAL, ativo: true } }),
      prisma.produto.create({ data: { empresaId: empresa.id, codigoInterno: 'MOV-002', codigoBarras: '7891234560081', gtin: '7891234560081', nome: 'Mesa Escritório 120cm', descricao: 'Mesa de escritório com gavetas', categoriaId: catMoveis.id, unidadeMedidaId: un.id, precoVenda: 650.00, precoCusto: 400.00, precoMinimo: 550.00, quantidadeEstoque: 8, estoqueMinimo: 2, estoqueMaximo: 15, ncm: '9403.30.00', cest: '01.004.00', origenMercadoria: OrigemMercadoria.NACIONAL, ativo: true } }),
      prisma.produto.create({ data: { empresaId: empresa.id, codigoInterno: 'ALI-001', codigoBarras: '7891234560098', gtin: '7891234560098', nome: 'Café Torrado 1kg', descricao: 'Café torrado e moído pacote 1kg', categoriaId: catAlimentos.id, unidadeMedidaId: un.id, precoVenda: 45.00, precoCusto: 25.00, precoMinimo: 38.00, quantidadeEstoque: 100, estoqueMinimo: 30, estoqueMaximo: 200, ncm: '0901.21.00', cest: '01.001.02', origenMercadoria: OrigemMercadoria.NACIONAL, ativo: true } }),
    ]);
  ...
  // Create Pedidos de Venda (only if not exist)
  ...
  if (existingPedidos === 0 && produtos.length >= 2) {
    await prisma.pedidoVenda.create({
      data: {
        ...
        numeroPedido: 'PV-2026-0001', serie: '1', tipoOperacao: 'VENDA', situacao: SituacaoPedido.CONFIRMADO,
  ...
  // Create Fluxo de Caixa (only if not exist)
  const existingFluxo = await prisma.fluxoCaixa.count({ where: { empresaId: empresa.id } });

  if (existingFluxo === 0) {
    await Promise.all([
      prisma.fluxoCaixa.create({ data: { empresaId: empresa.id, tipo: TipoFluxoCaixa.ENTRADA, categoria: 'Vendas', descricao: 'Venda Notebook Dell', valor: 3500.00, formaPagamento: FormaPagamento.DINHEIRO, dataMovimentacao: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000) } }),
      prisma.fluxoCaixa.create({ data: { empresaId: empresa.id, tipo: TipoFluxoCaixa.ENTRADA, categoria: 'Vendas', descricao: 'Venda Fones Bluetooth', valor: 500.00, formaPagamento: FormaPagamento.PIX, dataMovimentacao: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000) } }),
      prisma.fluxoCaixa.create({ data: { empresaId: empresa.id, tipo: TipoFluxoCaixa.SAIDA, categoria: 'Fornecedores', descricao: 'Pagamento Fornecedor Tech', valor: 2800.00, formaPagamento: FormaPagamento.TRANSFERENCIA, dataMovimentacao: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) } }),
      prisma.fluxoCaixa.create({ data: { empresaId: empresa.id, tipo: TipoFluxoCaixa.SAIDA, categoria: 'Aluguel', descricao: 'Aluguel Escritório Março', valor: 5500.00, formaPagamento: FormaPagamento.TRANSFERENCIA, dataMovimentacao: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000) } }),
    ]);
