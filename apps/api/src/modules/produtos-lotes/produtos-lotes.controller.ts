...
import { criarProdutoLoteSchema, atualizarProdutoLoteSchema, produtoLoteFiltroSchema, ajustarEstoqueSchema } from './dto/produto-lote.dto';
import { AuthRequest } from '@/shared/middleware/auth.middleware';

export class ProdutosLotesController {
  constructor(private service: ProdutosLotesService) {}

  criar = async (req: AuthRequest, res: Response) => {
    const empresaId = req.usuario?.empresaId || '';
    try {
      const dados = criarProdutoLoteSchema.parse(req.body);
  ...
  buscarPorId = async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (!resultado) return res.status(404).json({ error: 'Lote não encontrado' });
  ...
  listar = async (req: AuthRequest, res: Response) => {
    const empresaId = req.usuario?.empresaId || '';
    try {
      const filtros = produtoLoteFiltroSchema.parse({
        ...req.query,
  ...
  atualizar = async (req: AuthRequest, res: Response) => {
    ...
    const empresaId = req.usuario?.empresaId || '';
    try {
      const dados = atualizarProdutoLoteSchema.parse(req.body);
  ...
  ajustarEstoque = async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
    const empresaId = req.usuario?.empresaId || '';
    try {
      const dados = ajustarEstoqueSchema.parse(req.body);
      const resultado = await this.service.ajustarEstoque(id, empresaId, dados);
      return res.json(resultado);
    } catch (error: any) {
      return res.status(400).json({ error: error.message || error.issues?.[0]?.message || 'Validation error' });
    }
